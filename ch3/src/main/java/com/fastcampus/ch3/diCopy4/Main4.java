...
class AppContext {
    ...
    private void doComponentScan() {
        try {
            ...
            // 2. 반복문으로 클래스를 하나씩 읽어와서 @Component이 뭍어 있는지 확인
            // 3. @Component가 붙어잇으면 객체를 생성해서 map에 저장
...
public class Main4 {
    public static void main(String[] args) throws Exception {
        ...
        Car car = (Car) ac.getBean("car");  // buName 으로 객체 검색
        Engine engine = (Engine) ac.getBean("engine");
        Door door = (Door) ac.getBean(Door.class); // byType 으로 객체 검색
//        // 수동으로 객체 연결
//        car.engine = engine;
//        car.door = door;
